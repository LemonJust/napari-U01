...
class TableView(QtWidgets.QWidget):
    ...
    def populate(self, data_dict):
        ...
        for row, (id_, classification) in enumerate(data_dict.items()):
            ...
            class_ = classification['class']
            self.table.setItem(row, 0, QtWidgets.QTableWidgetItem(str(id_)))
            self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(class_))
            self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(subclass))
...
class LabelClassificationView:
    ...
    def update_label_layers(self, label, old_class_name=None):
        ...
        self.class_colormaps[label_class][label] = self.class_colors[
            label_class]
        segmentation_layer.color = self.class_colormaps[label_class]
    ...
    def highlight_label(self, label):
        ...
        # select the unclassified layer again
    ...
    def hide_segmentation_layers(self):
        self.update_visible_layers()
        self.segmentation_hidden = True

        for layer in self.viewer.layers:
            if layer.name in ['unclassified', 'neuron', 'glia',
                              'background', HL_NAME]:
                layer.visible = False

    def restore_layers_visibility(self):
        self.segmentation_hidden = False
        for layer in self.viewer.layers:
            if layer.name in self.visible_layers:
                layer.visible = True
